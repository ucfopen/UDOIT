				...
				<div class="errorSummary panel-body">
					<?php if (count($report->error) > 0): ?>
						<div class="panel panel-danger">
							...
							<ul class="list-group">
								<?php
									// build groups of errors

									$instanceIndices = []; // array of ints
									$indice = 0; // index of groups of error types
									$previtemtype = '';
									$instance = 1;
									$issue_count = 0;
									...
									foreach ($report->error as $item) {
										...
										if ($currtype != $previtemtype) {
											$instanceIndices[$indice] = $instance - 1;
											$newItemType = true;
											$instance = 1;
											$indice++;
										}

										$previtemtype = $currtype;
										$newItemType = false;
										$instance++;
									}
									...
									foreach ($report->error as $item) {
										...
										<?php if($newItemType): ?>
											...
											<?php endif; ?>
										<?php endif; ?>
										<!-- End Error Type Group Header -->

										<?php if ( in_array($item->type, $supported_error_types) ): ?>
										...
										<div id="collapse-<?= $report->id; ?>-<?= $issue_count; ?>" class="collapse in fade margin-top-small">
												...
												<div class="more-info hidden instance">
													<div class="error-preview">
														<?php if ($item->type == "videosEmbeddedOrLinkedNeedCaptions"): ?>
															<iframe width="100%" height="300px" src="https://www.youtube.com/embed/<?= $isYoutubeVideo($item->html, $regex); ?>" frameborder="0" allowfullscreen></iframe>
														<?php else: ?>
															<?= $item->html; ?>
														<?php endif; ?>
													</div>
											...
											<?php if (empty($post_path) && in_array($item->type, $supported_error_types)): ?>
