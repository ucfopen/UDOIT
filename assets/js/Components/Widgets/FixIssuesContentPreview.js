...
export default function FixIssuesContentPreview({
  ...
  const handlePressedElement = (e) => {
     if(!(e.code == "Space") && !(e.code == "Enter")){
      console.log("Cannot press another key besides space or space")
      return;
     }
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const tempElement = convertErrorHtmlString(Html.getIssueHtml(tempIssue?.issueData))
                ...
                { taggedContent && activeContentItem && !contentItemsBeingScanned.includes(activeContentItem?.id) ? (
                  <>
                    { canShowPreview ? (
                      <>
                        <div
                          className={FORM_CLASSIFICATIONS.CLICKABLE_RELATED.includes(formNameFromRule(activeIssue.scanRuleId)) ? `ufixit-content-preview-main ufixit-clickable-container` : `ufixit-content-preview-main`}
                          id='ufixit-content-preview-main'
                          onScroll={() => {
                            checkScrollButton()
                          }}
                          onClick={handleClickedElement}
                          onKeyDown={handlePressedElement}
